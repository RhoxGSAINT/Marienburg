...
local jaan_details = {
    ...
    unit_list = "wh3_dlc25_emp_veh_marienburg_land_ship,hkrul_privateers,hkrul_mar_inf_goedendagers,hkrul_mar_inf_greatsword,hkrul_mar_inf_boogschutter,hkrul_carriers_ror,hkrul_mar_inf_goedendagers", -- unit keys from main_units table
    region_key = "",
...
local function cr_change_marienburg_campaign_display()
	...
	cm:override_building_chain_display("wh_main_EMPIRE_settlement_major", "cr_mar_special_settlement_marienburg", "wh3_main_chaos_region_marienburg")
end
...
local function hkrul_mar()
	...
	if cm:is_new_game() then
        --cr_change_marienburg_campaign_display() --it does not look good in ROC
        cm:callback(
			function() 
